...
# Evaluation data extraction commands
                ...
                for line in f:
                    ...
                    if 'FileObject' in current_type or 'FILE_OBJECT' in current_type:
                        ...
                        if filename_search:
                            ...
                            # For target dataset: Ensure node existence with N/A
                            fileobject_info_file.write(f"{uuid}, filename: N/A\n")
                    ...
                    if 'com.bbn.tc.schema.avro.cdm20.Event' in line:
                        ...
                        if len(dstId1_list) > 0 and dstId1_list[0] != 'null':
                            ...
                            if row:
                                ...
                                fw.write(this_edge1)
                                total_edges += 1
                                # Collect file names for target dataset
                                if 'FileObject' in dstType1 or 'FILE_OBJECT' in dstType1:
                                    path_search = re.search(r'\"predicateObjectPath\"\s*:\s*\{\s*\"string\"\s*:\s*\"(.*?)\"', line)
                                    if not path_search: path_search = re.search(r'\"predicateObjectPath\"\s*:\s*\"(.*?)\"', line)
                                    if path_search:
                                        fileobject_info_file.write(f"{dstId1}, filename: {path_search.group(1)}\n")
                        ...
                        if len(dstId2_list) > 0 and dstId2_list[0] != 'null' and '00000000-0000' not in dstId2_list[0]:
                            ...
                            if row:
                                ...
                                total_edges += 1
                                # Collect file names for target dataset
                                if 'FileObject' in dstType2 or 'FILE_OBJECT' in dstType2:
                                    path_search2 = re.search(r'\"predicateObject2Path\"\s*:\s*\{\s*\"string\"\s*:\s*\"(.*?)\"', line)
                                    if not path_search2: path_search2 = re.search(r'\"predicateObject2Path\"\s*:\s*\"(.*?)\"', line)
                                    if path_search2:
                                        fileobject_info_file.write(f"{dstId2}, filename: {path_search2.group(1)}\n")
